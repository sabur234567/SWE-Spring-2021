...
class BaseAppService(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            responseBody['data'] = 'We-Go Vehicle Request API'

        elif '/supply-back-end/request_api/v1/all_requests' in path:
            database = DatabaseUtil()
            try:
                dispatches = database.read_dispatches()
                responseBody['data'] = dispatches
                status = self.HTTP_STATUS_RESPONSE_CODES['OK'].value
        ...
        elif '/supply-back-end/request_api/v1/requests' in path:
            order_id = paramsDict['order_id']
            database = DatabaseUtil()
            try:
                dispatches = database.read_dispatch(order_id)
                responseBody['data'] = dispatches
                status = self.HTTP_STATUS_RESPONSE_CODES['OK'].value
                responseBody['status'] = 'Success'
            except Exception as e:
    ...
    def do_POST(self):
        ...
        responseBody = {}
        if path == '/supply-back-end/request_api/v1/request':
            order = Order(postBody["user_id"], postBody["service_type"], postBody["address"], values=postBody)

            dispatch = Dispatch(order)
            dispatch.start_processing()
            complete_order = dispatch.complete_dispatch()

            database = DatabaseUtil()
            try:
                database.insert_dispatch(dispatch.__dict__())
                responseBody['status'] = 'Success'
                status = self.HTTP_STATUS_RESPONSE_CODES['OK'].value
            except Exception as e:
                responseBody['status'] = 'Failed'
            ...
            return_order = complete_order.__dict__()
            return_order['route'] = dispatch.route
            return_order['eta'] = dispatch.eta
            responseBody['data'] = json.dumps(return_order)
