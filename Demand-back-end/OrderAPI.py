...
class BaseAppService(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # http://localhost:8084.
        if path == '/':
            responseBody['data'] = 'We-Go Order API'
            status = self.HTTP_STATUS_RESPONSE_CODES['OK'].value

        elif '/demand-back-end/order_api/v1/all_orders' in path:
            database = DatabaseUtil()
            try:
                orders = database.read_orders()
        ...
        elif '/demand-back-end/order_api/v1/order' in path:
            user_id = paramsDict['user_id']
            database = DatabaseUtil()
            try:
                orders = database.read_order(user_id)
                responseBody['data'] = orders
                status = self.HTTP_STATUS_RESPONSE_CODES['OK'].value
                responseBody['status'] = 'Success'
            except Exception as e:
    ...
    def do_POST(self):
        ...
        if path == '/demand-back-end/order_api/v1/orders':
            order = Order(postBody['user_id'], postBody['service_type'], postBody['address'])
            dispatch = requests.post("https://supply.team11.sweispring21.tk/supply-back-end/request_api/v1/request",
                                     json.dumps(order.__dict__()))
            complete_order = dispatch.json()
            complete_order = json.loads(complete_order['data'])
            print('complete order', complete_order)
            responseBody['data'] = complete_order

            database = DatabaseUtil()
            try:
                database.insert_order(complete_order)
                responseBody['status'] = 'Success'
                status = self.HTTP_STATUS_RESPONSE_CODES['OK'].value
            except Exception as e:
                responseBody['status'] = 'Failed'
